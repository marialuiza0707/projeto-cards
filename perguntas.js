criaCartao(
    'Perfumes',
    'como são feitos os perfumes?',
    'Os perfumes são compostos basicamente de:mistura de fragrância (óleos esssenciais que formam os odore básicos do perfume, lcembrando que nenhum é perfume composto somente de um único aroma), solventes (geralmente o etanol, que, por sua vez, contém certa quantidade de água) e fixadores.'
)

criaCartao(
    'celular',
    'por quem foi desenvolvido o celular',
    'Foi desenvolvido pelo engenheiro Martin Cooper, nos Estados Unidos.'
)

criaCartao(
    'roupas',
    'como são feitas as roupas?',
    'Os materiais naturais incluem pele,couro ou fios naturais como aqueles encontrados no algodão ou em ovelhas. Materiais artificiais incluem nylon, poliéster e lycra. '
)

criaCartao(
    'dinheiro',
    'como é feito o dinheiro?',
    'É constituido por três camadas sobrepostas. A lâmina do meio é feita de 100% algodão, como algumas roupas que usamos no dia a dia. Já as camadas externas são feitas, normalmente, de pasta de madeira.'
)

criaCartao(
    'Gloss',
    'De onde surgiu o gloss?',
    'Max Factor em 1930 que queria criar um produto labial para dar brilho aos lábios dos atores em filmes.'
)



criaCartao(
    'esporte',
    'de onde surgiu o esporte?',
    ' O esporte surgiu da necessidade que o ser humano possuí nos primórdios: correr, pescar nadar e saltar.'
)

criaCartao(
    'Academia',
    'Qual a necesidade de praticar academia?',
    'É fundamental para a prevenção de doenças crônicas não transmissíveis.'
)

criaCartao(
    '',
    '?',
    ''
)

criaCartao(
    'dinheiro',
    'como é feito o dinheiro?',
    'É constituido por três camadas sobrepostas. A lâmina do meio é feita de 100% algodão, como algumas roupas que usamos no dia a dia. Já as camadas externas são feitas, normalmente, de pasta de madeira.'
)




